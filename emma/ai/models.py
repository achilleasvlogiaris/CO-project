...
from keras.layers import Dense, Activation, Input, Conv1D, Reshape, MaxPool1D, Flatten, LeakyReLU, PReLU, Dropout
...
class AI:
    ...
    def train_generator(self, training_iterator, validation_iterator, epochs=2000, workers=1, save=True):
        print('DEBUGDEBUGDEBUGDEBUGDEBUGDEBUGDEBUGDEBUGDEBUG train_generator')
    ...
    #                  rank_trace_step=1000, conf=None):
    #     print('DEBUGDEBUGDEBUGDEBUGDEBUGDEBUGDEBUGDEBUGDEBUG train_t_fold')
    #     """
    ...
    #     print(inputs.shape)
    #     print(labels.shape)
    #     print('training_iterator.num_total_examples', training_iterator.num_total_examples)
    #     print('num_train_traces', num_train_traces)
    #
    #     num_validation_traces = abs(training_iterator.num_total_examples - num_train_traces)
    #     print('num_validation_traces', num_validation_traces)
    ...
    #
    #         print("debug models")
    #         print(shuffled_inputs_train)
    #         print(shuffled_labels_train)
    ...
    #         print('shuffled_traces', shuffled_traces)
    #         print('shuffled_traces', len(shuffled_traces))
    #         print('num_train_traces', num_train_traces)
    #         print('training_iterator.num_total_examples', training_iterator.num_total_examples)
    #         # validation_traces = shuffled_traces[num_train_traces:]
    #         print(shuffled_traces.shape[0] - 100)
    #         # breakpoint()
    #         validation_traces = shuffled_traces[shuffled_traces.shape[0] - 100:]
    #         print('validation_traces', validation_traces)
    #         # for j in range(0, int(num_validation_traces / rank_trace_step)):
    #         #     print(int(num_validation_traces / rank_trace_step))
    #         for j in range(0, 500):
    #             validation_traces_subset = validation_traces[0:(j + 1) * rank_trace_step]
    #             # print('validation_traces_subset', validation_traces_subset)
    ...
    def train_t_fold(self, training_iterator, validation_iterator, batch_size=10000, epochs=100, num_train_traces=45000,
                     t=10,
                     rank_trace_step=1000, conf=None):
        """
        t-fold cross-validation according to paper by Prouff et al.
        ...
        # First, process all ops and apply them to the traces set
        all_traces = training_iterator.get_all_as_trace_set()
        validation_traces = validation_iterator.get_all_as_trace_set()

        # Use the preprocessing function of the iterator to convert to Keras features
        inputs, labels = training_iterator._preprocess_trace_set(all_traces)
        inputs_validation, labels_validation = validation_iterator._preprocess_trace_set(validation_traces)
        ...
        for i in range(0, t):
            ...
            # # Randomize inputs_validation and labels_labels validation in the same order
            # assert (len(inputs_validation) == len(labels_validation) == len(validation_traces.traces))
            # random_indices = np.arange(len(inputs_validation))
            # np.random.shuffle(random_indices)
            # shuffled_inputs_validation = np.take(inputs_validation, random_indices, axis=0)  # Take random input examples
            # shuffled_labels_validation = np.take(labels_validation, random_indices, axis=0)  # Take random label examples
            # shuffled_traces_validation = np.take(validation_traces.traces, random_indices, axis=0)
            # assert (labels_validation[random_indices[0]][0] == shuffled_labels_validation[0][0])
            # assert (shuffled_traces_validation[0].signal[0] == shuffled_inputs_validation[0][0])
            #
            # # breakpoint()

            shuffled_inputs_train = shuffled_inputs[0:np.shape(shuffled_inputs)[0] - num_validation_traces]
            # shuffled_inputs_train = shuffled_inputs
            shuffled_labels_train = shuffled_labels[0:np.shape(shuffled_inputs)[0] - num_validation_traces]
            # shuffled_labels_train = shuffled_labels

            shuffled_inputs_val = shuffled_inputs[np.shape(shuffled_inputs)[0] - num_validation_traces:]
            # shuffled_inputs_val = shuffled_inputs_validation
            shuffled_labels_val = shuffled_labels[np.shape(shuffled_inputs)[0] - num_validation_traces:]
            # shuffled_labels_val = shuffled_labels_validation
            ...
            # Now, evaluate the rank for increasing number of traces from the validation set (steps of 10)
            print(np.shape(shuffled_traces))
            print(training_iterator.num_total_examples)
            print(num_train_traces)
            print(len(all_traces.traces))
            # breakpoint()
            validation_traces = shuffled_traces[np.shape(shuffled_inputs)[0] - num_validation_traces:]
            # validation_traces = inputs_validation[:num_validation_traces]
            # validation_traces = shuffled_traces[shuffled_traces.shape[0] - 100:]
            for j in range(0, int(num_validation_traces / rank_trace_step)):
                validation_traces_subset = validation_traces[0:(j + 1) * rank_trace_step]
                x = np.array([trace.signal for trace in validation_traces_subset])
                # x = np.array([trace for trace in validation_traces_subset])
                if conf.cnn:
                    x = np.expand_dims(x, axis=-1)
                encodings = self.model.predict(x)  # Output: [?, 16]
    ...
    def predict(self, x):
        ...
        if self.model_type == 'autoenc':
            get_encode_layer_output = K.function([self.model.layers[0].input],
                                                 [self.model.layers[
                                                      1].output])  # TODO hardcoded "encode" layer index for autoenc
    ...
    def info(self):
        ...
        result += "Loss     : %s\n" % self.model.loss if isinstance(self.model.loss,
                                                                    str) else self.model.loss.__name__.strip()
        result += "Optimizer: %s\n" % self.model.optimizer if isinstance(self.model.optimizer,
                                                                         str) else self.model.optimizer.__class__.__name__.strip()
...
class AICorrNet(AI):
    def __init__(self, conf, input_dim, name="aicorrnet"):
        ...
        if self.cnn:
            ...
            optimizer = tf.keras.optimizers.Nadam(lr=conf.lr)
        ...
        if not self.cnn:
            ...
            for i in range(0, self.n_hidden_layers):
                ...
                hidden_nodes_list = [256, 128, 64, 32, 16, 8, 4, 2]
                hidden_nodes = hidden_nodes_list[i]
                self.model.add(Dense(hidden_nodes, input_dim=input_dim, use_bias=self.use_bias, activation=None,
                                     kernel_initializer=initializer,
                                     kernel_regularizer=str_to_reg(self.reg, self.reg_lambda)))
            ...
            self.model.add(
                Dense(LeakageModel.get_num_outputs(conf) + extra_outputs, input_dim=input_dim, use_bias=self.use_bias,
                      activation=None, kernel_initializer=initializer, kernel_constraint=constraint,
                      kernel_regularizer=str_to_reg(self.regfinal, self.reg_lambda)))
            if self.batch_norm:
        ...
        # self.model.compile(optimizer=optimizer, loss=self.loss, metrics=[], run_eagerly=True)
        ...
        if not conf.norank:
            self.callbacks['rank'] = rankcallbacks.CorrRankCallback(conf,
                                                                    '/tmp/keras/' + self.name + '-' + self.id + '/rank/',
                                                                    save_best=True, save_path=self.model_path)
    ...
    def train_set(self, x, y, save=False, epochs=1, extra_callbacks=[]):
        ...
        # Fit model
        self.model.fit(x, y, epochs=epochs, batch_size=999999999, shuffle=False, verbose=2,
                       callbacks=[last_loss, history] + extra_callbacks)
...
class AISHACPU(AI):
    def __init__(self, conf, input_shape, name="aishacpu", subtype='vgg16'):
        ...
        self.model = None
        if subtype == 'vgg16':
            self.model = VGG16(include_top=True, weights=None, input_tensor=input_tensor, input_shape=None,
                               pooling='avg', classes=9 if self.hamming else 256)
        elif subtype == 'custom':
            self.model = Sequential()
            ...
            self.model.add(
                Dense(9 if self.hamming else 256, use_bias=True, input_shape=input_shape, kernel_regularizer=self.reg))
            self.model.add(BatchNormalization(momentum=0.1))
